...
- JSON Schema constraints declared on a spec's schema attributes (enum, const, minLength/maxLength, pattern, minimum/maximum, minItems/maxItems, enum-constrained string collection elements) are carried end to end and emitted as standard `terraform-plugin-framework-validators` calls on managed-resource and provider-config attributes (e.g. `stringvalidator.OneOf`, `int64validator.Between`, `listvalidator.ValueStringsAre`). A declared bound of `0` is preserved (`minimum: 0` forbids negative values). Placeholder values in generated configuration text (docs examples, acceptance-test configs, `.tftest.hcl` modules) are derived from the same constraints — const, first enum member, bounds-clamped, length-adjusted, or a deterministic pattern candidate — so a generated config satisfies the validators generated from the same schema; the residual gap is a `pattern` no candidate on the deterministic list matches, where the placeholder stays `"example"` and must be substituted by hand. Data sources, actions, ephemeral resources, list resources, and functions carry the constraints in the IR but do not yet emit validators; wiring them is follow-up work (see `PROJECT_DESIGN.md` §23.2).
...
- Secret-named attributes in action and list-resource schemas cannot be marked `Sensitive` (the framework's `action/schema` rejects it and the experimental `list/schema` has no Sensitive support), so their values surface in plain text. Eidos emits a fail-loud Warning per attribute at generation time and renders a practitioner-facing warning on the affected doc page naming the attributes. Managed resources, data sources, ephemeral resources, and the provider config do mark such attributes `Sensitive`.
- A resource whose read takes no path parameters and no required query/header parameters (a singleton, e.g. `GET /copilot/config`) is importable with any identifier: the import stores the raw ID in the identity attribute and the refresh that follows repopulates state from the response. The generation output notes this via an Info diagnostic.
