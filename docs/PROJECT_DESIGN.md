...
| Nested managed-resource identities | A primitive response field nested one object level below the resource (for example `metadata.name` or `dashboard.uid`) is promoted when it uniquely matches a path parameter and no top-level field matches. `ir.AttributeIR.WirePath` keeps request and response JSON nested while Terraform exposes the identity at the top level; unrelated wrapper fields remain nested. Ambiguous matches emit a Warning and remain unwired instead of guessing. |
