...
func ValidatorsFile(pir ir.ProviderIR) File {
	file := generateValidatorsFile(pir)
	if file == nil {
...
// validatorNeeds tracks which custom validator implementations are required
// for a given provider IR. A zero value means no custom validators are needed.
...
// collectValidatorNeeds walks the provider IR and records which custom
// validators are required by any schema.
