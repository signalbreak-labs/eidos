...
func ModelFile(r ir.ResourceIR) File {
	path := filepath.Join("internal", "provider", fmt.Sprintf("model_%s.go", snakeCase(r.Name)))
	return GoCodeAST(path, generateModelFile(r))
}
...
// resourceAPIModelName returns the generated API model struct name for a resource.
func resourceAPIModelName(r ir.ResourceIR) string {
	return pascalCase(r.Name) + "Model"
}
