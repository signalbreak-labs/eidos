...
// with a non-overlapping trigger (workflow_dispatch) so the two coexist.
func DynamicReleaseWorkflow(image, specPath string) File {
...
# generation.dynamic_release in generator.yaml.
...
func FilesForProviderIR(provider *ir.ProviderIR, cfg BuildConfig, opts CollectOptions) ([]File, error) {
	...
	if opts.IncludeDynamicRelease {
		files = append(files, DynamicReleaseWorkflow(opts.DynamicReleaseImage, opts.DynamicReleaseSpecPath))
	}
