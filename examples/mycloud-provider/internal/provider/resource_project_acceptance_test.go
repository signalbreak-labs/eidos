...
func TestAccProjectResourceLifecycle(t *testing.T) {
	t.Setenv("TF_ACC", "1")
	server := newProjectResourceMockServer()
	defer server.Close()
	resource.Test(t, resource.TestCase{ProtoV6ProviderFactories: map[string]func() (tfprotov6.ProviderServer, error){"mycloud": providerserver.NewProtocol6WithError(New())}, Steps: []resource.TestStep{resource.TestStep{Config: testAccProjectResourceConfig(server.URL, "example"), Check: resource.ComposeAggregateTestCheckFunc(resource.TestCheckResourceAttrSet("mycloud_project.example", "id"), resource.TestCheckResourceAttr("mycloud_project.example", "default_branch", "example"))}, resource.TestStep{Config: testAccProjectResourceConfig(server.URL, "updated"), Check: resource.ComposeAggregateTestCheckFunc(resource.TestCheckResourceAttrSet("mycloud_project.example", "id"), resource.TestCheckResourceAttr("mycloud_project.example", "default_branch", "updated"))}, resource.TestStep{ResourceName: "mycloud_project.example", ImportState: true, ImportStateId: "imported-organization:imported-project"}}})
}
