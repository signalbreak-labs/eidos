	...
	stream.Results = func(push func(list.ListResult) bool) {
		...
		for _, item := range items {
			...
			if err := json.Unmarshal(item, &itemMap); err != nil {
				result.Diagnostics.AddError("Error listing mycloud_list_pull_requests", fmt.Sprintf("Could not decode list item: %s", err))
				if !push(result) {
					return
				}
				continue
...
func (l *ListPullRequestsListResource) listRemote(ctx context.Context, config *ListPullRequestsListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing mycloud_list_pull_requests", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &items); err != nil {
			diags.AddError("Error listing mycloud_list_pull_requests", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
