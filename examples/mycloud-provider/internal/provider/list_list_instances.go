...
func (l *ListInstancesListResource) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	stream.Results = func(push func(list.ListResult) bool) {
		...
		if err != nil {
			{
				pushError("Error listing mycloud_list_instances", fmt.Sprintf("Could not read list response: %s", err))
				return
			}
		}
		for _, page := range pages {
			...
			if err := json.Unmarshal(page, &items); err != nil {
				pushError("Error listing mycloud_list_instances", fmt.Sprintf("Could not decode list page: %s", err))
				return
			}
