...
func (l *ListProjectsForOrganizationListResource) listRemote(ctx context.Context, config *ListProjectsForOrganizationListResourceModel) ([]json.RawMessage, diag.Diagnostics) {
	...
	if err != nil {
		diags.AddError("Error listing mycloud_list_projects_for_organization", fmt.Sprintf("Could not read list response: %s", err))
		return nil, diags
	}
	...
	for _, page := range pages {
		...
		if err := json.Unmarshal(page, &items); err != nil {
			diags.AddError("Error listing mycloud_list_projects_for_organization", fmt.Sprintf("Could not decode list page: %s", err))
			return nil, diags
		}
