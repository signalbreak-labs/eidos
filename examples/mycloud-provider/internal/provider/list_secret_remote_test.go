...
// TestSecretListResource_List_APIError exercises SecretListResource.listRemote against an httptest mock: non-success status surfaces Could not read list response carrying the API error status and body.
func TestSecretListResource_List_APIError(t *testing.T) {
	r := &SecretListResource{client: newMockClientStatus(t, 500, "{\"message\":\"boom\"}")}
	m := SecretListResourceModel{}
	_, diags := r.listRemote(context.Background(), &m)
