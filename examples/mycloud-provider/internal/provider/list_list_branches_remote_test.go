...
// TestListBranchesListResource_List_APIError exercises ListBranchesListResource.listRemote against an httptest mock: non-success status surfaces Could not read list response carrying the API error status and body.
func TestListBranchesListResource_List_APIError(t *testing.T) {
	r := &ListBranchesListResource{client: newMockClientStatus(t, 500, "{\"message\":\"boom\"}")}
	m := ListBranchesListResourceModel{}
	_, diags := r.listRemote(context.Background(), &m)
