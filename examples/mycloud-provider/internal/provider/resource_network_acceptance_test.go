...
func TestAccNetworkResourceLifecycle(t *testing.T) {
	...
	defer server.Close()
	resource.Test(t, resource.TestCase{ProtoV6ProviderFactories: map[string]func() (tfprotov6.ProviderServer, error){"mycloud": providerserver.NewProtocol6WithError(New())}, Steps: []resource.TestStep{resource.TestStep{Config: testAccNetworkResourceConfig(server.URL, "example"), Check: resource.ComposeAggregateTestCheckFunc(resource.TestCheckResourceAttrSet("mycloud_network.example", "id"), resource.TestCheckResourceAttr("mycloud_network.example", "api_version", "example"))}, resource.TestStep{Config: testAccNetworkResourceConfig(server.URL, "updated"), Check: resource.ComposeAggregateTestCheckFunc(resource.TestCheckResourceAttrSet("mycloud_network.example", "id"), resource.TestCheckResourceAttr("mycloud_network.example", "api_version", "updated"))}, resource.TestStep{ResourceName: "mycloud_network.example", ImportState: true, ImportStateId: "imported-workspace:imported-name"}}})
}
