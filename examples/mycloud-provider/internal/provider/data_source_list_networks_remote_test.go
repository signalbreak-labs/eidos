...
// TestListNetworksDataSource_Read_APIError exercises ListNetworksDataSource.readListRemote against an httptest mock: non-success status surfaces Could not read list response carrying the API error status and body.
func TestListNetworksDataSource_Read_APIError(t *testing.T) {
	r := &ListNetworksDataSource{client: newMockClientStatus(t, 500, "{\"message\":\"boom\"}")}
	m := ListNetworksDataSourceModel{}
	resp := &datasource.ReadResponse{}
	r.readListRemote(context.Background(), &m, resp)
