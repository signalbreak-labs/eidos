...
// TestListStacksDataSource_Read_APIError exercises ListStacksDataSource.readListRemote against an httptest mock: non-success status surfaces Could not read list response carrying the API error status and body.
func TestListStacksDataSource_Read_APIError(t *testing.T) {
	r := &ListStacksDataSource{client: newMockClientStatus(t, 500, "{\"message\":\"boom\"}")}
	m := ListStacksDataSourceModel{}
	resp := &datasource.ReadResponse{}
	r.readListRemote(context.Background(), &m, resp)
