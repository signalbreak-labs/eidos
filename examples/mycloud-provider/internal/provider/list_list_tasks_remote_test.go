...
// TestListTasksListResource_List_APIError exercises ListTasksListResource.listRemote against an httptest mock: non-success status surfaces Could not read list response carrying the API error status and body.
func TestListTasksListResource_List_APIError(t *testing.T) {
	r := &ListTasksListResource{client: newMockClientStatus(t, 500, "{\"message\":\"boom\"}")}
	m := ListTasksListResourceModel{}
	_, diags := r.listRemote(context.Background(), &m)
