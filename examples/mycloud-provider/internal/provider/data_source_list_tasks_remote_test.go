...
// TestListTasksDataSource_Read_APIError exercises ListTasksDataSource.readListRemote against an httptest mock: non-success status surfaces Could not read list response carrying the API error status and body.
func TestListTasksDataSource_Read_APIError(t *testing.T) {
	r := &ListTasksDataSource{client: newMockClientStatus(t, 500, "{\"message\":\"boom\"}")}
	m := ListTasksDataSourceModel{}
	resp := &datasource.ReadResponse{}
	r.readListRemote(context.Background(), &m, resp)
