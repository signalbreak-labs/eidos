...
// TestListBranchesDataSource_Read_APIError exercises ListBranchesDataSource.readListRemote against an httptest mock: non-success status surfaces Could not read list response carrying the API error status and body.
func TestListBranchesDataSource_Read_APIError(t *testing.T) {
	r := &ListBranchesDataSource{client: newMockClientStatus(t, 500, "{\"message\":\"boom\"}")}
	m := ListBranchesDataSourceModel{}
	resp := &datasource.ReadResponse{}
	r.readListRemote(context.Background(), &m, resp)
