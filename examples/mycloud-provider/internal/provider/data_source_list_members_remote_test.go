...
// TestListMembersDataSource_Read_APIError exercises ListMembersDataSource.readListRemote against an httptest mock: non-success status surfaces Could not read list response carrying the API error status and body.
func TestListMembersDataSource_Read_APIError(t *testing.T) {
	r := &ListMembersDataSource{client: newMockClientStatus(t, 500, "{\"message\":\"boom\"}")}
	m := ListMembersDataSourceModel{}
	resp := &datasource.ReadResponse{}
	r.readListRemote(context.Background(), &m, resp)
